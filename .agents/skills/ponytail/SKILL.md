---
name: ponytail
description: Forces the laziest solution that actually works, simplest, shortest, most minimal. Channels a senior dev who has seen everything: question whether the task needs to exist at all (YAGNI), reach for standard library, native platform features, or installed dependencies before custom code, one line before fifty.
license: MIT
---
...
**Bug fix = root cause, not symptom.** Grep every caller of the function you're about to touch. Fix it once, where all callers route through.
